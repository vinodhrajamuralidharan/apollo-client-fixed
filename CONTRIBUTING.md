...
```
git clone https://github.com/apollographql/apollo-client-fixed.git
cd apollo-client-fixed
npm i
cd ..
```

> From this point forward `[apollo-client-fixed-root]` represents the root directory of your Apollo Client checkout (e.g. `/some/path/apollo-client-fixed`).
...
rm -Rf ./client
ln -s [apollo-client-fixed-root]/dist client
```
...
ln -s [apollo-client-fixed-root]/node_modules/react
ln -s [apollo-client-fixed-root]/node_modules/react-dom
```
...
```
# ... assuming still in [apollo-client-fixed-root] from step 6
cd src
